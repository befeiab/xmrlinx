...
{    int daxiao= size;
      //printf("%s", m_sendBuf);
char base64[4096];char m_sendBuf1[4096];
	if (m_sendBuf[0] == '{')
	{
		
		if(m_sendBuf[daxiao - 1] = '\n') m_sendBuf[daxiao - 1] = '\0';
		memset(base64, 0, sizeof(base64));
            memset(m_sendBuf1, 0, sizeof(m_sendBuf1));
            //m_sendBuf1 = m_sendBuf.data();
            strcpy(m_sendBuf1, m_sendBuf.data());
			*(strrchr(m_sendBuf1, '}') + 1) = '\0';
		base64_encode(m_sendBuf1, base64, strlen(m_sendBuf1));
		memset(m_sendBuf1, 0, sizeof(m_sendBuf1));
		strcpy(m_sendBuf1, base64);
		strcat(m_sendBuf1, "\n");
           // *(strrchr(m_sendBuf, '\n') + 1) = '\0';
		daxiao = strlen(m_sendBuf1);
	}
          //m_sendBuf = m_sendBuf1;
		  size = daxiao;
    LOG_DEBUG("[%s] send (%d bytes): \"%.*s\"", url(), size, static_cast<int>(size) - 1, m_sendBuf1);
...
#   endif
    {
        ...
        uv_buf_t buf = uv_buf_init(m_sendBuf1, (unsigned int) daxiao);
...
void xmrig::Client::parse(char *line, size_t len)
{
    startTimeout();
        int daxiao = len;
     // printf("%s\n", line);
	if (line[0] == 'e'&&line[1] == 'y')
	{
		//line[len - 1] = '\0';
		//printf("%s\n", line);
		unsigned char bindata[2050];
		//char quhuanhang[4096];
		//strncpy(quhuanhang, line, strlen(line) - 1);
		memset(bindata, 0, sizeof(bindata));
		base64_decode(line, bindata);
           // printf("%s\n", line);
		memset(line, 0, sizeof(line));
		strcpy(line, (char*)bindata);
            //printf("%s\n", line);
		*(strrchr(line, '}') + 1) = '\0';
		strcat(line, "\n");
		//printf("%s\n", bindata);
